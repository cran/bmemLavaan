...
bmem.sem<-function(x, ram, data, N, robust=FALSE, varphi=.1,...){
  ##x:cov matrix, ram:model, data:raw data
  
  if (robust){
    # sem.res<-lavaan::sem(model = ram, data = data, sample.cov = x,sample.nobs = N, 
    #                      estimator="MLR", se="robust.huber.white",...)
    invisible(capture.output(rfit <- rsem::rsem.lavaan(dset = data, model = ram, varphi = varphi)))
    sem.res <- rfit$lavaan
  }
  else{
...
bmem.list<-function(x, ram, robust=FALSE, varphi=.1,...){
  ##x:data, ram:model
  N<-dim(x)[1]
  temp.cov<-bmem.list.cov(x)
  bmem.sem(temp.cov, ram, x, N, robust, varphi,...)
}

bmem.pair<-function(x, ram, robust=FALSE, varphi=.1,...){
  N<-dim(x)[1]
...
bmem.mi.cov<-function(x, m=10){
  mi.data<-Amelia::amelia(x,m,0)$imputations
  mi.cov<-list()
  for (j in 1:m){
...
bmem.mi<-function(x, ram, v, m=10, robust=FALSE, varphi=.1,...){
  N<-dim(x)[1]
  ...
  for (j in 1:m){
    ...
    temp<-bmem.sem(v.cov, ram, x, N, robust, varphi,...)
...
bmem.em<-function(x, ram, v, robust=FALSE, varphi=.1, st='i', max_it=500, ...){
  ...
  bmem.sem(temp.cov, ram, N=N, ...)
}

bmem.list.jack<-function(x, ram, robust=FALSE, varphi=.1, ...){
  n<-dim(x)[1]
  nseq<-1:n
  jack.est<-NULL
  jack.fit<-NULL
  for (i in 1:n){
    x.jack<-x[nseq[-i],]
    jack.temp<-try(bmem.list(x.jack, ram,robust, varphi, ...))
...
bmem.pair.jack<-function(x, ram, robust=FALSE, varphi=.1, ...){
  n<-dim(x)[1]
  nseq<-1:n
  jack.est<-NULL
  jack.fit<-NULL
  for (i in 1:n){
    x.jack<-x[nseq[-i],]
    jack.temp<-try(bmem.pair(x.jack, ram,robust, varphi, ...))
    if (class(jack.temp)!="try-error"){
      jack.est<-rbind(jack.est, jack.temp$est)
      jack.fit<-rbind(jack.fit, jack.temp$model.fit)
    }
...
bmem.mi.jack<-function(x, ram, v, m=10, robust=FALSE, varphi=.1, ...){
  n<-dim(x)[1]
  nseq<-1:n
  jack.est<-NULL
  jack.fit<-NULL
  for (i in 1:n){
    x.jack<-x[nseq[-i],]
    jack.temp<-try(bmem.mi(x.jack, ram, v, m, robust, varphi, ...))
    if (class(jack.temp)!="try-error"){
      jack.est<-rbind(jack.est, jack.temp$est)
      jack.fit<-rbind(jack.fit, jack.temp$model.fit)
    }
...
bmem.list.boot<-function(x, ram, boot=1000,robust=FALSE, varphi=.1, parallel=FALSE,ncore = 1,...){	
  # x:data, ram:model
  model0<-bmem.list(x, ram, robust, varphi,...)
  par0<-model0$est
  ...
  myfun <- function(i){
    ...
    modelb<-try(bmem.list(x.boot, ram, robust, varphi,...),silent=TRUE) ## silent=TRUE
  ...
  {if(parallel){ ## needs package "snowfall" and "parallel"
    ...
    snowfall::sfLibrary("bmemLavaan", character.only = TRUE)
    ...
    for (i in 1:boot){
      x.boot<-x[sample(n,n, replace=TRUE),]
      modelb<-try(bmem.list(x.boot, ram, robust, ...),silent=TRUE) ##silent=TRUE
      if (class(modelb)!="try-error"){
...
bmem.pair.boot<-function(x, ram, boot=1000, robust=FALSE, varphi=.1, parallel=FALSE,ncore = 1,...){
  model0<-bmem.pair(x, ram, robust, varphi,...)
  par0<-model0$est
  fit0<-model0$model.fit
  n<-dim(x)[1]
  ...
  myfun <- function(i){
    x.boot<-x[sample(n,n, replace=TRUE),]
    modelb<-try(bmem.pair(x.boot, ram, robust, varphi,...), silent=TRUE) ## silent=TRUE
    if (class(modelb)!="try-error"){
      return(list(est=modelb$est,fit=modelb$model.fit))  ## use result[[i]]$est,result[[i]]$fit to get the ith result  
    }
  }
  ...
  {if(parallel){ ## needs package "snowfall" and "parallel"
    ...
    for (i in 1:boot){
      x.boot<-x[sample(n,n, replace=TRUE),]
      modelb<-try(bmem.pair(x.boot, ram,robust, ...),silent=TRUE) ##silent=TRUE
      if (class(modelb)!="try-error"){
        boot.est<-rbind(boot.est, modelb$est)
        boot.fit<-rbind(boot.fit, modelb$model.fit)
      }
...
bmem.mi.boot<-function(x, ram, v, m=10, boot=1000, robust=FALSE, varphi=.1, parallel=FALSE,ncore = 1,...){
  model0<-bmem.mi(x, ram, v, m, robust, varphi,...)
  par0<-model0$est
  fit0<-model0$model.fit
  n<-dim(x)[1]
  ...
  myfun <- function(i){
    x.boot<-x[sample(n,n, replace=TRUE),]
    modelb<-try(bmem.mi(x.boot, ram, v, m, robust, varphi,...),silent=TRUE) ## silent=TRUE
    if (class(modelb)!="try-error"){
      return(list(est=modelb$est,fit=modelb$model.fit))  ## use result[[i]]$est,result[[i]]$fit to get the ith result  
    }
  }
  ...
  {if(parallel){ ## needs package "snowfall" and "parallel"
    snowfall::sfInit(parallel = TRUE, cpus = ncore)# detectCores() - 1)
    snowfall::sfLibrary("lavaan", character.only = TRUE)
    snowfall::sfLibrary("Amelia", character.only = TRUE)
    snowfall::sfLibrary("bmemLavaan", character.only = TRUE)
...
bmem.em.boot<-function(x, ram, v, robust=FALSE, varphi=.1, st='i', boot=1000, max_it=500, parallel=FALSE,ncore = 1,...){
  ...
  myfun <- function(i){
    x.boot<-x[sample(n,n, replace=TRUE),]
    modelb<-try(bmem.em(x.boot, ram, robust = robust, ...),silent=TRUE) ## silent=TRUE
    if (class(modelb)!="try-error"){
      return(list(est=modelb$est,fit=modelb$model.fit))  ## use result[[i]]$est,result[[i]]$fit to get the ith result  
    }
  }
...
bmem.ci.norm<-function(par.boot, par0, cl=.95){
  ...
  se.boot<-apply(par.boot, 2, sd, na.rm=TRUE)
  estimate<-par0
  ci<-rep(1,np)%*%t(estimate)+qnorm(alpha)%*%t(se.boot)
...
bmem<-function(data, model, v, method='list', ci='perc', cl=.95, boot=1000, m=10, varphi=.1, st='i', robust=FALSE, max_it=500, parallel=FALSE, ncore=1, ...){
  ...
  P<-ncol(data)
  if (missing(v)) v<-1:P
  if (method!='list' & method!='pair' & method!='mi' & method!='tsml'){
    stop("Error: 'method' must be 'list', 'pair', 'mi', or 'tsml'")
  }
  if (ci!='norm' & ci!='perc' & ci!='bc' & ci!='bca'){
    stop("Error: 'ci' must be 'norm', 'perc', 'bc', or 'bca'")
  }
  
  ## for listwise deletion method
  if (method=='list'){
    data<-data[,v]
    boot.est<-bmem.list.boot(data, model, boot, robust, varphi, parallel, ncore, ...)
    if (ci=='norm'){
    ...
    if (ci=='bca'){
      jack.est<-bmem.list.jack(data,model, robust,...)
  ...
  if (method=='pair'){
    data<-data[,v]
    boot.est<-bmem.pair.boot(data, model, boot, robust, varphi, parallel, ncore,...)
    if (ci=='norm'){
      ci.est<-bmem.ci.norm(boot.est$par.boot, boot.est$par0, cl)
      ci.fit<-bmem.ci.norm(boot.est$boot.fit, boot.est$fit0, cl)
    }
    ...
    if (ci=='bca'){
      jack.est<-bmem.pair.jack(data,model, robust,...)
      ci.est<-bmem.ci.bca(boot.est$par.boot, boot.est$par0, jack.est$jack.est, cl)
      ci.fit<-bmem.ci.bca(boot.est$boot.fit, boot.est$fit0, jack.est$jack.fit, cl)
    }
  }
  ...
  if (method=='mi'){
    boot.est<-bmem.mi.boot(data,model,v,m,boot, robust, varphi, parallel, ncore,...)
    if (ci=='norm'){
      ci.est<-bmem.ci.norm(boot.est$par.boot, boot.est$par0, cl)
      ci.fit<-bmem.ci.norm(boot.est$boot.fit, boot.est$fit0, cl)
    }
    ...
    if (ci=='bca'){
      jack.est<-bmem.mi.jack(data,model,v,m, robust,...)
      ci.est<-bmem.ci.bca(boot.est$par.boot, boot.est$par0, jack.est$jack.est, cl)
      ci.fit<-bmem.ci.bca(boot.est$boot.fit, boot.est$fit0, jack.est$jack.fit, cl)
    }
  }
  ...
  lavpartable <- lavpartable[1:nrow(ci.est),]
  lavpartable[,"est"] <- ci.est[,"estimate"]
  lavpartable[,"se"] <- ci.est[,"se.boot"]
  
  if (method=='list') mis_method <- "listwise deletion"
  else{
    if (method=='pair') mis_method <- "pairwise deletion"
    else{
      if (method=='mi') mis_method <- "multiple imputation"
      else mis_method <- "two-stage maximum likelihood"
    }
  }
  infor <- list(samplesize=N,method=mis_method,boot=boot,sucboot=nrow(boot.est$par.boot),ci=ci)
...
bmem.bs<-function(x, ram, v, ci='bc', cl=.95, boot=1000, max_it=500, verbose=TRUE,...){
...
bmem.cov <- function(ram,obs.variables, debug=FALSE, verbose=TRUE){
  ##if (moment){
  ##fixed.x<-'(i)'
  #3}else{
  fixed.x=NULL
  ...
  if ((!is.matrix(ram)) | ncol(ram) != 3) stop ('Error: ram argument must be a 3-column matrix')
  ...
  if (df < 0) stop(paste("Error: the model has negative degrees of freedom =", df))
  J <- matrix(0, n, m)
